...
<form action="{{ route('admin.placements.update', $placement) }}" method="POST" class="space-y-6">
    ...
    <div class="bg-white p-4 border rounded space-y-4 max-w-xl">
        <div>
            ...
            <input name="name" value="{{ old('name', $placement->name) }}" class="mt-1 w-full border rounded px-3 py-2" required />
            @error('name')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium">Key (identifier)</label>
            <input name="key" value="{{ old('key', $placement->key) }}" class="mt-1 w-full border rounded px-3 py-2" required />
            @error('key')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium">Width</label>
                <input type="number" name="width" value="{{ old('width', $placement->width) }}" class="mt-1 w-full border rounded px-3 py-2" />
            </div>
            <div>
                <label class="block text-sm font-medium">Height</label>
                <input type="number" name="height" value="{{ old('height', $placement->height) }}" class="mt-1 w-full border rounded px-3 py-2" />
            </div>
        </div>
        <div>
            <label class="inline-flex items-center">
                <input type="checkbox" name="is_auto" value="1" class="mr-2" @checked(old('is_auto', $placement->is_auto)) />
                Automatic rendering
            </label>
        </div>
    </div>
