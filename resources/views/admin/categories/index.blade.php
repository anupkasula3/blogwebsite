        ...
        <table class="min-w-full bg-white rounded-lg shadow divide-y divide-gray-200">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Slug</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Posts</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Status</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Created</th>
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-purple-50 even:bg-gray-50 transition-all">
                    <td class="px-6 py-4 whitespace-nowrap flex items-center gap-3">
                        @if($category->image)
                        <img class="h-10 w-10 rounded-lg object-cover border" src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}">
                        @else
                    ...
                    <td class="px-6 py-4 text-right flex items-center justify-end gap-2">
                        <a href="{{ route('admin.categories.show', $category) }}" class="text-blue-600 hover:text-blue-800 bg-blue-50 rounded p-2 transition" title="View">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="{{ route('admin.categories.edit', $category) }}" class="text-indigo-600 hover:text-indigo-800 bg-indigo-50 rounded p-2 transition" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" class="inline delete-category-form">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="text-red-600 hover:text-red-800 bg-red-50 rounded p-2 transition delete-category-btn" title="Delete" data-category-name="{{ $category->name }}">
...
<div x-data="{ open: false, form: null, category: '' }" x-init="
...
" x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
    <div class="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full">
        ...
        <p class="mb-6 text-gray-700">Are you sure you want to delete the category <span class="font-semibold text-red-600" x-text="category"></span>? This will also delete all posts in this category.</p>
