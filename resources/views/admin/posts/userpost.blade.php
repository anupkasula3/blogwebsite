...
@section('content')
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($posts as $post)
                        <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">
                                            <a href="{{ route('post.show', $post->slug) }}" target="_blank"
                                                class="hover:text-purple-600">{{ $post->title }}</a>
                                        </div>
                            ...
                            <td class="px-6 py-4 ">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">{{ $post->category->name }}</span>
                            </td>
                            <td class="px-6 py-4 ">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $post->user->name ?? 'Unknown User' }}</div>
                                        <div class="flex items-center">
                                            <span
                            ...
                            <td class="px-6 py-4  text-sm text-gray-900">{{ number_format($post->views_count) }}</td>
                            <td class="px-6 py-4  text-sm text-gray-500">
                                {{ $post->published_at ? $post->published_at->format('M j, Y') : 'Not published' }}</td>
                            <td class="px-6 py-4  text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <a href="{{ route('admin.userposts.show', $post) }}"
                                        class="text-blue-600 hover:text-blue-900" title="View Full Post">
                                        <i class="fas fa-eye"></i>
                                    </a>

                                    <form method="POST" action="{{ route('admin.userposts.destroy', $post) }}"
                                        class="inline delete-userpost-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="text-red-600 hover:text-red-900 delete-userpost-btn"
    ...
    <div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <div class="mb-4">
                    <p class="text-sm text-gray-600 mb-2">Post: <span id="rejectPostTitle" class="font-medium"></span>
                    </p>
    ...
    <div x-data="{ open: false, form: null, post: '' }" x-init="document.querySelectorAll('.delete-userpost-btn').forEach(btn => {
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full">
            ...
            <p class="mb-6 text-gray-700">Are you sure you want to delete the post <span
                    class="font-semibold text-red-600" x-text="post"></span>? This action cannot be undone.</p>
