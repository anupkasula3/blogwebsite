...
<div class="bg-white rounded-xl shadow-lg p-8">
    <div class="flex items-center justify-between mb-6 border-b pb-4">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('admin.userposts.index') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <table class="min-w-full bg-white rounded-lg shadow divide-y divide-gray-200">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Published</th>
                    <th class="px-6 py-4 text-right text-xs font-bold text-gray-600 uppercase tracking-wider">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Post</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Category</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Author</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Status</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Views</th>
                    <th class="px-6 py-3 text-left font-bold text-gray-600 uppercase tracking-wider text-xs">Published</th>
                    <th class="px-6 py-3 text-right font-bold text-gray-600 uppercase tracking-wider text-xs">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr class="hover:bg-purple-50 even:bg-gray-50 transition-all">
                    <td class="px-6 py-4 whitespace-nowrap flex items-center gap-3">
                        ...
                        <img class="h-10 w-10 rounded-lg object-cover border" src="{{ Storage::url($post->featured_image) }}" alt="{{ $post->title }}">
                        @else
                        <div class="h-10 w-10 bg-gradient-to-r from-purple-600 to-blue-600 rounded-lg flex items-center justify-center">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">{{ $post->published_at ? $post->published_at->format('M j, Y') : '-' }}</td>
                    <td class="px-6 py-4 text-right flex items-center justify-end gap-2">
                        <a href="{{ route('admin.posts.show', $post) }}" class="text-blue-600 hover:text-blue-800 bg-blue-50 rounded p-2 transition" title="View">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="{{ route('admin.posts.edit', $post) }}" class="text-indigo-600 hover:text-indigo-800 bg-indigo-50 rounded p-2 transition" title="Edit">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form method="POST" action="{{ route('admin.posts.destroy', $post) }}" class="inline delete-post-form">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="text-red-600 hover:text-red-800 bg-red-50 rounded p-2 transition delete-post-btn" title="Delete" data-post-title="{{ $post->title }}">
