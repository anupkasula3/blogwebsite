...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-sm text-gray-600">Manage motivational quotes</p>
        </div>
        <div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Quote</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($quotes as $key => $quote)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600">{{ ($quotes->firstItem() ?? 0) + $key }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 max-w-2xl">{{ $quote->quote }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <form action="{{ route('admin.quotes.destroy', $quote) }}" method="POST" class="inline delete-quote-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" data-quote-id="{{ $quote->id }}" data-quote="{{ Str::limit($quote->quote, 60) }}" title="Delete"
                                            class="inline-flex delete-quote-btn items-center px-3 py-1.5 text-xs font-medium text-white rounded-lg bg-[#ff3131] hover:opacity-90 transition-colors">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 mr-1" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-6 text-center text-gray-500">No quotes found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-data="{ open: false, form: null, quote: '' }" x-init="document.querySelectorAll('.delete-quote-btn').forEach(btn => {
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full">
            ...
            <p class="mb-6 text-gray-700">Are you sure you want to delete the quote <span class="font-semibold text-red-600" x-text="quote"></span>? This action cannot be undone.</p>
