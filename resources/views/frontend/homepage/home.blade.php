...
@section('content')
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="bg-light">
                        ...
                        <div class="lg:col-span-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach ($latestPosts->skip(2)->take(6) as $key => $post)
                                    <article
                                        class="bg-white rounded-lg shadow-md overflow-hidden news-card transition-all duration-300 border border-gray-200">
                                        <div class="relative">
                                            <img src="{{ asset('uploads/' . $post->featured_image) }}"
                                                alt="{{ $post->title }}" class="w-full h-40 sm:h-48 object-cover">
                                            <div class="absolute top-3 left-3">
                                                <span
                                                    class="bg-black bg-opacity-75 text-white px-2 py-1 text-xs font-semibold rounded">
                                                    {{ $post->category->name }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="p-4">
                                            <div class="flex items-center gap-2 mb-2">
                                                <span
                                                    class="text-xs text-primary font-semibold">{{ $post->published_at->format('M j') }}</span>
                                                <span class="text-xs text-gray-400">•</span>
                                                <span
                                                    class="text-xs text-gray-500">{{ $post->published_at->diffForHumans() }}</span>
                                            </div>
                                            <h4
                                                class="font-bold text-gray-900 mb-2 leading-tight hover:text-primary transition-colors">
                                                <a
                                                    href="{{ route('post.show', $post->slug) }}">{{ Str::limit($post->title, 80) }}</a>
                                            </h4>
                        ...
                        @foreach ($popularPosts->take(8) as $key => $post)
                            <article
                                ...
                                <div class="relative">
                                    ...
                                    <div class="absolute top-3 left-3">
                                        <div
                                            class="bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">
                                            {{ $key + 1 }}
                                        </div>
                                    </div>
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex items-center gap-2 mb-3">
                                        <span
                                            class="text-xs text-blue-600 font-semibold">{{ $post->published_at->format('M j, Y') }}</span>
                                        <span class="text-xs text-gray-400">•</span>
                                        <span
                                            class="text-xs text-gray-500">{{ $post->published_at->diffForHumans() }}</span>
                                    </div>
                                    ...
                                    <!-- Title -->
                                    <h4
                                        class="font-bold text-gray-900 mb-3 leading-tight group-hover:text-blue-600 transition-colors">
                                        <a
                                            href="{{ route('post.show', $post->slug) }}">{{ Str::limit($post->title, 70) }}</a>
                                    </h4>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-xs text-gray-500">{{ Str::limit($post->author_name, 15) }}</span>
                                        </div>
                                    ...
                                    @if ($key < 3)
                                        <div class="mt-3 flex items-center gap-2">
                                            <div
                                                class="flex-1 bg-gradient-to-r from-red-500 to-orange-500 h-1 rounded-full">
                                            </div>
            ...
            <section class=" mx-auto max-w-screen-2xl px-4 text-black relative overflow-hidden ">
                <div class="">
                    ...
                    <div class="flex flex-col lg:flex-row gap-3">
                        ...
                        <div class="w-full lg:w-1/3 xl:w-2/5 space-y-3">
                            ...
                            <div class="flex flex-col lg:flex-row gap-3">
                                ...
                                <!-- Newsletter Signup -->
                                <div class="bg-primary w-1/2 rounded-lg p-6 text-white">
                                    <div class="text-center">
                                        <i class="fas fa-newspaper text-3xl mb-3"></i>
                                        <h4 class="font-bold text-lg mb-2">Daily Newsletter</h4>
                                        <p class="text-red-100 text-sm mb-4">Get breaking news delivered to your inbox</p>
                                ...
                                <div
                                    class="bg-gradient-to-b  w-1/2 from-purple-100 to-blue-100 rounded-lg p-4 border-2 border-dashed border-purple-300">
                                    <div class="text-center">
                                        <span class="text-xs text-gray-500 uppercase tracking-wide">Advertisement</span>
                                        @if ($sidebarAd)
                                            <div class="mt-2">
            ...
            <section class="py-8 bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-100 relative overflow-hidden">
                <div class="max-w-screen-2xl mx-auto px-4">
                    ...
                    <div class="flex justify-center">
                        <a href="{{ route('all-posts') }}"
                            class="inline-flex items-center px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-lg font-semibold rounded-full shadow-lg hover:from-purple-600 hover:to-indigo-600 transition-all duration-200">
                            Explore All Posts
            ...
            <section class="py-8">
                <div class="max-w-screen-2xl mx-auto px-4">
                    <div
                        class="max-w-4xl mx-auto bg-gradient-to-r from-blue-100 via-indigo-100 to-purple-100 rounded-3xl shadow-xl flex flex-col md:flex-row items-center gap-6 p-6 md:p-10 border border-blue-200 relative overflow-hidden">
                        <div class="flex-shrink-0 w-full md:w-1/3 flex justify-center">
                            <img src="{{ asset('images/adddds.jpg') }}" alt="Special Offer Banner"
                                class="h-32 md:h-40 w-auto object-contain rounded-2xl shadow-lg">
                        </div>
                        <div class="flex-1 text-center md:text-left">
                            <h3 class="text-2xl md:text-3xl font-extrabold text-blue-900 mb-2">Unlock Exclusive Content!
                            </h3>
                            ...
                            <a href="{{ route('register') }}"
                                class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-base font-semibold rounded-full shadow hover:from-purple-600 hover:to-indigo-600 transition-all duration-200">
                                Get Started
                                <i class="fas fa-arrow-right ml-2"></i>
                            </a>
                        </div>
