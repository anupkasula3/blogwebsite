...
@if ($hero)
    <div class="relative rounded-2xl overflow-hidden shadow-lg mb-8">
        <img src="{{ asset('uploads/' . $hero->featured_image) }}" alt="{{ $hero->title }}"
            class="w-full h-[28rem] object-cover">
        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent"></div>
        <div class="absolute bottom-0 left-0 p-10 text-white max-w-2xl">
            <span
                class="inline-block bg-gradient-to-r from-[#ff2953] to-[#ff6883] px-3 py-1 rounded-full text-xs font-semibold mb-4">{{ $hero->category->name }}</span>
            ...
            <a href="{{ route('post.show', $hero->slug) }}"
                class="inline-block bg-[#ff2953] hover:bg-[#e02448] px-8 py-3 rounded-lg font-semibold text-lg shadow-lg transition">Read
                More</a>
        </div>
    </div>
@endif
