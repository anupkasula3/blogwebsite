    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center">
                ...
                <h1 class="hidden lg:block text-2xl font-bold text-gray-800" x-data
                    x-text="document.title.split(' - ')[0]"></h1>
            </div>

            <div class="flex items-center space-x-4">

                {{-- <div class="relative" x-data="{ open: false, notifications: [], unreadCount: 0 }" x-init="fetch('/api/notifications/unread-count')
                 ...
                 fetch('/api/notifications/latest')
                    ...
                    <div x-show="open" @click.away="open = false" x-transition
                        class="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 z-50 origin-top-right">
                        ...
                        <div class="p-4 bg-gray-50 border-t border-gray-200">
                            <form action="{{ route('user.notifications.read-all') }}" method="POST" class="inline">
                                @csrf
                                <button type="submit"
                                    class="w-full text-sm text-center text-blue-600 hover:text-blue-800 font-medium">Mark
                                    all as read</button>
                            </form>
                        </div>
                    </div>
