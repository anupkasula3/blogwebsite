        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div>
                <a href="{{ route('user.posts.index') }}"
                    class="text-gray-500 hover:text-purple-600 font-medium mb-2 inline-flex items-center">
            ...
            <div class="flex items-center space-x-4 mt-4 sm:mt-0">
                <a href="{{ route('post.show', $post->slug) }}" target="_blank"
                    class="px-4 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 font-medium inline-flex items-center">
                    <i class="fas fa-external-link-alt mr-2"></i>
                    View Live
                </a>
                <a href="{{ route('user.posts.edit', $post) }}"
                    class="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-6 py-2.5 rounded-lg shadow-lg hover:opacity-90 transition-opacity font-semibold inline-flex items-center">
            ...
            <div class="col-span-12 xl:col-span-4">
                <div class="space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-lg">
                        ...
                        <ul class="space-y-3 text-sm">
                            <li class="flex items-center justify-between">
                                <span class="text-gray-600 font-medium flex items-center"><i
                                        class="fas fa-eye mr-2 text-gray-400"></i>Views</span>
                                <span class="font-bold text-gray-800">{{ number_format($post->views_count) }}</span>
                            </li>
                            <li class="flex items-center justify-between">
                                <span class="text-gray-600 font-medium flex items-center"><i
                                        class="fas fa-clock mr-2 text-gray-400"></i>Reading Time</span>
                    ...
                    <div class="bg-red-50 p-6 rounded-2xl shadow-lg border border-red-200">
                        ...
                        <button type="button"
                            class="w-full bg-red-600 text-white px-4 py-2.5 rounded-lg hover:bg-red-700 transition-colors font-semibold"
    ...
    <div id="delete-post-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 sm:p-8 max-w-sm w-full mx-4"
            ...
            <form action="{{ route('user.posts.destroy', $post) }}" method="POST" class="flex justify-end gap-3">
                @csrf
                @method('DELETE')
                <button type="button" onclick="document.getElementById('delete-post-modal').classList.add('hidden')"
                    class="px-4 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 font-medium">Cancel</button>
                <button type="submit"
                    class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 font-medium">Delete Forever</button>
            </form>
        </div>
    </div>
@endsection
