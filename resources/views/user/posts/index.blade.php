...
@section('content')
                            ...
                            <tr>
                                <td class="p-4">
                                    <div class="flex items-center">
                                        ...
                                        @else
                                            <div
                                                class="w-10 h-10 rounded-lg bg-gray-200 flex items-center justify-center mr-3">
                                                <i class="fas fa-image text-gray-400"></i>
                                            </div>
                                        @endif
                                        <div>
                                            <div class="font-semibold text-gray-800">{{ $post->title }}</div>
                                            <div class="text-xs text-gray-500">{{ $post->category->name }} &bull;
                ...
                @forelse($posts as $post)
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center mb-3">
                            ...
                            <div>
                                <a href="{{ route('user.posts.show', $post) }}"
                                    class="font-semibold text-gray-800 hover:text-purple-600">{{ $post->title }}</a>
                                <div class="text-xs text-gray-500">{{ $post->category->name }} &bull;
                                    {{ $post->created_at->format('M d, Y') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            ...
                            @elseif($post->status == 'published' || $post->status == 'pending')
                                <form action="{{ route('user.posts.draft', $post) }}" method="POST" class="inline-block">
                                    @csrf
                                    <button type="submit" class="text-gray-500 hover:text-yellow-600"
                                        title="Move to Drafts">
                                        <i class="fas fa-download"></i>
                                    </button>
                                </form>
                            @endif
                            <a href="{{ route('user.posts.show', $post) }}" class="text-gray-500 hover:text-blue-600"
                                title="View"><i class="fas fa-eye"></i> View</a>
                            <a href="{{ route('user.posts.edit', $post) }}" class="text-gray-500 hover:text-indigo-600"
                                title="Edit"><i class="fas fa-edit"></i> Edit</a>
                            <button class="text-gray-500 hover:text-red-600 delete-post-btn" title="Delete"
                                data-post-title="{{ $post->title }}"
                                data-action="{{ route('user.posts.destroy', $post) }}"><i class="fas fa-trash"></i>
                                Delete</button>
                        </div>
                    </div>
                @empty
    ...
    <div x-data="{ open: false, action: '', title: '' }"
        @delete-post.window="open = true; action = $event.detail.action; title = $event.detail.title" x-show="open"
        style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
        ...
        <div @click.away="open = false" class="bg-white rounded-lg shadow-lg p-6 sm:p-8 max-w-sm w-full mx-4"
            ...
            <p class="mb-6 text-gray-700">Are you sure you want to delete the post <strong x-text="title"></strong>? This
                action cannot be undone.</p>
            <form :action="action" method="POST" class="flex justify-end gap-3">
                @csrf
                @method('DELETE')
                <button type="button" @click="open = false"
                    class="px-4 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 font-medium">Cancel</button>
                <button type="submit"
                    class="px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 font-medium">Delete</button>
            </form>
        </div>
    </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.delete-post-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        window.dispatchEvent(new CustomEvent('delete-post', {
                            detail
                        }));
                    });
                });
